        ...
        <div className='w-full flex flex-col items-center gap-[5rem]'>
            <div className='h-[calc(100vh-var(--h-navbar))] w-screen relative'>
                ...
                <div className='w-full h-full relative flex md:flex-row flex-col gap-[3rem] md:gap-[5rem] lg:gap-[10rem] items-center pb-[10rem] pl-[2rem] md:pl-[4rem] lg:pl-[6rem] p-[2rem]'>
                    <div className='flex flex-col gap-[1rem] justify-center md:justify-normal'>
                        <h1 className='text-3xl md:text-5xl font-bold text-center md:text-left'>{text.title}</h1>
                        <p className='text-sm md:text-lg text-center md:text-left'>{text.description}</p>
                        <div className='flex flex-col xs:flex-row gap-[1rem]'>
                            <Link href={'/recipes/'} className='flex justify-center'>
                                <Button className='cursor-pointer bg-green-600 hover:bg-green-500'>{text.exploreRecipes}</Button>
                            </Link>
                            <Link href={'/'} className='flex justify-center'>
                                <Button className='cursor-pointer hover:text-green-600 text-green-600' variant={'outline'}>{text.favoriteRecipes}</Button>
                            </Link>
                        </div>
                    </div>
            ...
            <div className='flex flex-col gap-[10rem]'>
                ...
                <div className='flex flex-col items-center gap-10 w-full'>
                    <h1 className='capitalize text-center text-xl font-semibold'>{text.sortByCategory}</h1>
...
async function ContentType({limit}:{limit:number}){
    ...
    if (typeof recipes === 'string') {
        return <div>{text.errorFetchingRecipes}</div>
    }
